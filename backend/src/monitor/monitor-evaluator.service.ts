...
export class MonitorEvaluatorService {
  ...
  private calculateCounters(monitor: Monitor, outcome: ProbeOutcome) {
    ...
    const isSpeedTest =
      monitor.type === MonitorType.HTTP || monitor.type === MonitorType.PING;

    if (outcome.latencyMs != null && isSpeedTest) {
      const isWarning =
        monitor.latencyWarningMs != null &&
        outcome.latencyMs >= monitor.latencyWarningMs;
      const isCritical =
    ...
    return {
      consecutiveFailures: !outcome.ok
        ? (monitor.consecutiveFailures ?? 0) + 1
        : 0,
      consecutiveSuccesses: outcome.ok
