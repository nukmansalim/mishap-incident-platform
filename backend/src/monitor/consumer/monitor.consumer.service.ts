...
export class MonitorConsumerService extends WorkerHost {
  ...
  async handleProbingCheck(job: Job) {
    try {
      ...
      await this.stateService.persistEvaluation(
        monitor.id,
        outcome,
        evaluation,
      );
      this.logger.debug(
        `[TICK] ${monitor.name} | Status Next: ${evaluation.nextStatus}`,
      );
    } catch (error: unknown) {
      this.logger.error(
        `Gagal mengeksekusi tick untuk monitor ${job.data.monitorId}: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
}
