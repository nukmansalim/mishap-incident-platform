...
export class MonitorProbeService {
    ...
    async executeProbe(monitor: Monitor): Promise<ProbeOutcome> {
        ...
        if (!strategy) {
            ...
            return { ok: false, errorMessage: `Tipe monitor tidak didukung: ${monitor.type}` };
        }

        try {
            return await strategy.probe(monitor);
        } catch (error: any) {
            this.logger.error(`Probe gagal secara fatal untuk monitor ${monitor.id}: ${error.message}`);
